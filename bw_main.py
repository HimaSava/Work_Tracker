...
#Global Variables
# topics = ["Capstone","DIP Research","Streben","IELTS","AEC Course","College Stuff","Projects"]
topics = []
topStatus = []
topTime = []
flag_screen2 = 0
...
def screen1():
    ...
    titleFrame.pack()
    titleFrame.config(bg="white")
    welcomeLabel = Label(titleFrame, text="Good Morning", font =('arial', 20, 'bold'), bg="white", fg="black")
    ...
    startButton = Button(titleFrame, text="Start DAY!!", command=startButton_command, 
                            ...
                            font = ('arial', 10, 'bold'), activebackground="black", activeforeground="white")
...
def screen2():
    ...
    workFrame.pack()
    workFrame.config(bg="white")
    dateLabel = Label(workFrame, text="Date:", font =('arial', 14, 'bold'), bg="white", fg="black")
    dateoutLabel = Label(workFrame, text="########", font =('arial', 14, 'bold'), bg="white", fg="black")
    timeLabel = Label(workFrame, text="Time:", font =('arial', 14, 'bold'), bg="white", fg="black")
    timeoutLabel = Label(workFrame, text="########", font =('arial', 14, 'bold'), bg="white", fg="black")
    ...
    activityFrame.pack()
    activityFrame.config(bg="white")
    HeadLabel = Label(activityFrame, text="Current Activity:", font =('arial', 10, 'normal'), bg="white", fg="black")
    HeadLabel.grid(row=0, column=0, padx=10, pady=10)
    
    buttonMenu = OptionMenu(activityFrame, clicked, *topics)
    buttonMenu.config( bg="white", fg="black", font = ('arial', 12, 'normal'), activebackground="black")
    buttonMenu.grid(row=0, column=1, padx=10, pady=10)

    statusLabel = Label(activityFrame, text="Status:", font =('arial', 12, 'normal'), bg="white", fg="black")
    statusLabel.grid(row=0, column=2, padx=10, pady=10)
    statusoutLabel = Label(activityFrame, text="########", font =('arial', 12, 'normal'), bg="white", fg="black")
    ...
    topicFrame.pack()
    topicFrame.config(bg="white")

    Label(topicFrame, text="Topic", font =('arial', 14, 'bold'), bg="white", fg="black").grid(row=0, column=0, sticky=W, padx=40)
    Label(topicFrame, text="Status", font =('arial', 14, 'bold'), bg="white", fg="black").grid(row=0, column=1, sticky=W, padx=20)
    Label(topicFrame, text="Time", font =('arial', 14, 'bold'), bg="white", fg="black").grid(row=0, column=2, sticky=W, padx=20)
    
    status = []
    time = []

    for i in range(len(topics)):
        Label(topicFrame, text=topics[i], font =('arial', 12, 'normal'), bg="white", fg="black").grid(row=i+1, column=0, sticky=W, padx=40)
        status.append(Label(topicFrame, text=topStatus[i], font =('arial', 12, 'normal'), bg="white", fg="black"))
        status[i].grid(row=i+1, column=1, sticky=W)
        time.append(Label(topicFrame, text=topTime[i].strftime("%H:%M:%S"), font =('arial', 12, 'normal'), bg="white", fg="black"))
    ...
    addTopicFrame.pack()
    addTopicFrame.config(bg="white")
    addTopicLabel = Label(addTopicFrame, text="Add Topic:", font =('arial', 12, 'bold'), bg="white", fg="black")
    addTopicLabel.grid(row=0, column=0, padx=10, pady=10)
    addTopicEntry = Entry(addTopicFrame, width=20, bg="white", fg="white")
    ...
    if flag_screen2 == 0:
        ...
        for i in range(len(topics)):
            ...
            if topStatus[i] == "In Progress":
                ...
                status[i].config(text=topStatus[i], font=('arial',12,'normal'),fg="black")
    ...
    if On == True:
        for i in range(len(topics)):
            if activeTopic == topics[i]:
                ...
                seconds = spentTime.seconds + pastTime.second
                topTime[i] = datetime.time(hour, minute, seconds)
                actionButton.config(text="Start", fg="black")
